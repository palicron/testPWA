...
class PasswordField extends Component {
  ...
  validatePasswordStrong = value => {
    ...
    if (value.length <= this.thresholdLength) throw new Error("Contraseña muy corta");

    // ensure password is strong enough using the zxcvbn library
    if (zxcvbn(value).score < this.minStrength) throw new Error("Contraseña débil");
  };
      ...
      <Fragment>
        <div className="position-relative">
          ...
          <FormField type="password" validator={this.validatePasswordStrong} onStateChanged={this.stateChanged} {...restProps}>
            <span className="d-block form-hint">La contraseña debe contener más de 7 caracteres.</span>
            {children}
